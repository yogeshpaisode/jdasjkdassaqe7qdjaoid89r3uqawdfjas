(function () {
    module.controller("inboxParentListController", ["$scope", "INBOXList", "settingVal", "$window", "$interval", function ($scope, INBOXList, settingVal, $window, $interval) {
        ...
        $scope.fetchMailList = function () {
            ...
            INBOXList.query(function (response) {
                $scope.inboxMailList = $scope.inboxMailList.concat(response);
                $scope.fetchingIsInProgress = false;
                console.log("Success Fetching Inbox Mail List " + angular.toJson($scope.inboxMailList));
            }, function (response) {
                $scope.fetchingIsInProgress = false;
                $scope.gotIssue = true;
                retrying();
